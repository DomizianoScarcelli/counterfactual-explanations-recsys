...
csv_path = "results/stats/alignment/targeted_fidelity_filtered.csv"
...
filtered_df = df[df["gen_target_y@1"].isin([cat2id[cat] for cat in categories])]

# Generate markdown tables
md_content = ""
for cat in categories:
    cat_id = cat2id[cat]
    row = filtered_df[filtered_df["gen_target_y@1"] == cat_id]
    if not row.empty:
...
file_path = "/mnt/data/fidelity_scores.md"
