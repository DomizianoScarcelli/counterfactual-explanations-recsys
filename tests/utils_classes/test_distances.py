...
def test_JaccardSim_ReturnsOne_WhenSetsAreIdentical():
    a = tensor([1, 2, 3])
    b = tensor([1, 2, 3])
    assert jaccard_sim(a, b) == 1.0, "Failed for identical sets."

def test_JaccardSim_ReturnsZero_WhenSetsAreDisjoint():
    a = tensor([1, 2, 3])
    b = tensor([4, 5, 6])
    assert jaccard_sim(a, b) == 0.0, "Failed for disjoint sets."

def test_PrecisionAtK_ReturnsCorrectValue_WhenPartialMatchOccurs():
    a = tensor([1, 2, 3, 4, 5])
    b = tensor([1, 2, 6, 7, 8])
    assert precision_at(3, a, b) == 2 / 3, "Failed for partial match."

def test_PrecisionAtK_ReturnsZero_WhenNoItemsMatch():
    a = tensor([1, 2, 3, 4, 5])
    b = tensor([6, 7, 8, 9, 10])
    assert precision_at(3, a, b) == 0.0, "Failed for no overlap."

def test_PrecisionAtK_ReturnsOne_WhenAllTopKItemsMatch():
    a = tensor([1, 2, 3, 4, 5])
    b = tensor([1, 2, 3, 6, 7])
    assert precision_at(3, a, b) == 1.0, "Failed for perfect match."

def test_NDCGAtK_ReturnsOne_WhenPerfectRankingIsProvided():
    a = tensor([1, 2, 3, 4, 5])
    b = tensor([1, 2, 3, 6, 7])
    assert torch.isclose(torch.tensor(ndcg_at(3, a, b)), tensor([1.0])), "Failed for perfect ranking."

def test_NDCGAtK_ReturnsCorrectValue_WhenRankingIsPartial():
    a = tensor([1, 2, 3, 4, 5])
    b = tensor([3, 2, 1, 6, 7])
...
def test_NDCGAtK_ReturnsZero_WhenNoRelevantItemsAreRanked():
    a = tensor([1, 2, 3, 4, 5])
    b = tensor([6, 7, 8, 9, 10])
