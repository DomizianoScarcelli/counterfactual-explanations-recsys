...
eval_batch_size = 1
config = Config(model='BERT4Rec', dataset='ml-100k', config_dict={"train_neg_sample_args": None, "eval_batch_size": eval_batch_size, "checkpoint_dir": "saved/BERT4Rec-Oct-01-2024_17-27-29.pth"})
...
checkpoint_file = "saved/BERT4Rec-Oct-01-2024_17-27-29.pth"
...
def generate_counterfactual_dataset(interaction: Interaction) -> List[Interaction]:
    ...
    def model_predict(seq):
        ...
        #Pad with 0s
        seq = torch.cat((seq, torch.zeros((MAX_LENGTH - len(seq))))).to(seq.dtype)

        seq = seq.unsqueeze(0)
        new_interaction = deepcopy(interaction)
        new_interaction.interaction["item_id_list"] = seq
        return predict(model, new_interaction).item()
