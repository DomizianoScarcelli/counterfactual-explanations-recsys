...
def predict(
    model: SequentialRecommender, seq: Tensor, argmax: bool = True
) -> torch.Tensor:
...
def model_predict(
    seq: torch.Tensor, model: SequentialRecommender, prob: bool = True
) -> Tensor | float:
...
def model_batch_predict(
    ...
    assert (
        len(interactions) == batch_size
    ), f"Interaction len must be equal to batch size {len(interactions)} != {batch_size}"
    preds = torch.tensor([])
